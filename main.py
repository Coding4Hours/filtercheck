...
def lightspeed(url: str) -> list[str | int]:
    ...
    try:
        ...
        body_json = res.json()
        return lightspeed_categorize(body_json["data"]["a"]["cat"])
...
def fortiguard(url: str) -> str:
    ...
    try:
        res = requests.post(
            "https://www.fortiguard.com/learnmore/dns", headers=headers, json=body
        )
        r_json = res.json()
        cat = r_json["dns"]["categoryname"]
        cat_block = is_blocked(cat, "fortiguard")
        return (
            f"{cat} ({'Likely **Blocked**' if cat_block else 'Likely **Unblocked**'})"
        )
    except Exception as e:
        return f"Error: {e}"
...
def palo(domain: str) -> list[str] | str:
    try:
        res = requests.get(
            f"https://urlfiltering.paloaltonetworks.com/single_cr/?url={domain}"
        )
        soup = BeautifulSoup(res.text, "html.parser")
        cat_arr = [
            soup.select('*[for="id_new_category"]')[2]
            .parent.find(class_="form-text")
            .text.strip(),
            soup.select('*[for="id_new_category"]')[1]
            .parent.find(class_="form-text")
            .text.strip(),
        ]
        cat = cat_arr[0]
...
def blocksi(domain: str) -> str | int:
    try:
        r = requests.get(
            f"https://service1.blocksi.net/getRating.json?url={domain}"
        ).json()
        if "Category" in r:
